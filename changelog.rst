...
* :meth:`iter` now returns :data:`iter(value)` instead of :data:`iter(elm)`.
...
* :meth:`remove_relations` now returns a bool, :class:`False` when error suppressed by :data:`err='ignore'`, :class:`True` when no errors occured, to match :meth:`delete`. Programs that need to support older versions should simply switch to :data:`iter(elm)`.
